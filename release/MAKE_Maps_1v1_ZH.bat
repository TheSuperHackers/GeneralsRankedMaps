
echo off
set FILENAME=Maps_1v1_ZH.zip
set MAPS=
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Abandoned Desert ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Ammars Sandcastles v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Annihilation/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Area J1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Arena of War ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Artic Lagoon/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Australia ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Barren Badlands Balanced ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Bitter Winter Balanced NoCars ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Blizzard Badlands Reloaded/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Blizzard Badlands ZHv5/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Bounty v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Bozic Destruction ZH v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Canyon of the Dead v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Coastal Conflict ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Cold Territory ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Danger Close ZH/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] DeDuSu ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Desert Quadrant ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Deserted Village v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Desolated District ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Devastated Oasis ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Eagle Eye/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Early Spring ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Eight ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Embattled Land ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Evergreen Lagoon/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Final Crusade Balanced ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Final Crusade FIXEDv2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Flash Fire Balanced ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Forbidden Takover ZH v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Forest of Oblivion ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Forgotten Air Battle v4/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Forgotten Ruins/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Frog Prince ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Frozen Ruins/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Gold Cobra/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Hard Winter ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Hidden Treasures v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Highway 99 ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Homeland Rocks ZH v4/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Irish Front ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Jungle Wolf ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Lagoon ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Lagoon ZH v4/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Liquid Gold ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Lost Valley v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Make Make 2 ZH v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Melting Snow ZH v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Mountain Mayhem v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Mountain Oil ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Natural Threats ZH v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Onza Map v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Rebellion ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Sand Scorpion/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Sand Serpent Balanced ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Sand Serpent FIXED/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Scaraa ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Scorched Earth ZH v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Silicon Valley ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Sleeping Dragon v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Snow Aggression v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Snow Blind ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Snowy Drought v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Snowy Drought v4/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Storm Valley/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD Classic NoCars ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD Classic ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD NoBugs ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD NoBugsCars ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD OpenMiddle NoCars ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD OpenMiddle ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD Resurrection V2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] TD Winter ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] The First Strike ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] The Path ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Total Domination No SDZ ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Total Domination SDZ ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Tournament Delta ZH v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Tournament in the Canyon vA/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Tournament in the Canyon vB/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Uneven Heights v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Uneven Heights v3/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Vendetta ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Wasteland Warlords Balanced ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Wasteland Warlords Revised/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] White Serpent/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Winter Arena/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Winter Crows ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] Winter Wolf Balanced ZH v1/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] ZH Carrier is Over v2/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Battle on the River/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Blasted Lands/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Gorge Drought/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Plant Waste/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Summer Arena/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Tournament Arena/*"
set MAPS=%MAPS% "maps_1v1_zh/[RANK] [NMC] Tournament City/*"
echo on

tar.exe -a -c -f %FILENAME% --strip-components=1 %MAPS%

pause